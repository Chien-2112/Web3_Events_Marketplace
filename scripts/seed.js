...
const generateEventData = (count) => {
	...
	for(let i = 0; i < count; i++) {
		...
		const event = {
			...
			sales: faker.number.int({ min: 1, max: 10 }),
...
async function main() {
	let dappEventCContract;

	try {
		const contractAddress = fs.readFileSync("./contracts/contractAddress.json", "utf8");
		const { dappEventCContract: dappEventCAddress } = JSON.parse(contractAddress);

		dappEventCContract = await ethers.getContractAt("DappEvents", dappEventCAddress);

		// Process 1.
		await Promise.all(
			generateEventData(dataCount).map(async (event) => {
				await createEvent(dappEventCContract, event);
			})
		)
				...
				.map(async (_, i) => {
					...
					await Promise.all(
						Array(randomCount)
							.fill()
							.map(async() => {
								await buyTickets(dappEventCContract, i + 1, 1)
							})
					)
				})
		...
		// Process 3.
		await getEvents(dappEventCContract);
		await getTickets(dappEventCContract, 1);
	} catch(error) {
